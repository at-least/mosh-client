//! The full hostile receive path, as SEQUENCES: length-prefixed
//! fragments feed one persistent assembly, and every assembled
//! instruction hits BOTH receivers — the server-side UserStream and
//! the client-side HostStreamState (HostMessage decode, the event
//! log, echo-ack merging). One input can carry a whole hostile
//! exchange: reference states, throwaways, out-of-order inserts,
//! duplicates (spec §6.2).
